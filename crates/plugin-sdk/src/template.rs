//! `{placeholder}` substitution for the configurable prompts and
//! instructions of task_source plugins (#318, #398, #759).
//!
//! Moved here from three identical plugin copies (github, notion, slack) —
//! plugins may depend only on `plugin-protocol` / `plugin-sdk`
//! (`scripts/arch-lint.sh`), so `orchestrator_core::template::render` is out
//! of their reach and one plugin cannot borrow another's. Core keeps its own
//! copy for the same reason in the other direction.

/// Substitute `{key}` tokens in a **single pass**: a substituted value that
/// itself contains `{token}` text is never re-expanded.
///
/// The single-pass property is load-bearing. Every caller substitutes values
/// that are, or sit next to, **content someone outside the operator
/// controls** — an issue body, a Notion page title, a Slack message. A second
/// pass would turn a `{placeholder}` written in that content into a
/// directive. Do not reach for a `loop { replace }`; each caller notes which
/// of its variables are the untrusted ones.
///
