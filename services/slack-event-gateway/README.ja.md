...
当てられるうえ、当たるまで何も起きない。OpenTofu モジュール経由で作った表でも、
Secret Manager に手で置いた表でも、短いトークンを含む表はコンテナが起動時に拒否する。
...
**同じトピックが表の中に 2 度出てはいけない。**列の中でも、列をまたいでもである。
2 人が同じトピックを指した表は何も言わずに起動し、そのあと片方に届いた配送 ——
チャンネル ID・`ts`・リアクション —— がもう片方のサブスクリプションに落ちる。
2 つの列はまとめて検査するので、「A の `topic` が B の `block_actions_topic`」も
通らない。その組み合わせは押下を保持期間が日単位のサブスクリプションに入れてしまう。
