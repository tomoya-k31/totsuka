...
//! task**, so host replies and the poll loop's background `task/submit`
...
//! the Keychain — its token arrives already resolved in `initialize` config
//! (F-65).

use task_source_github::server::{Server, TransportFactory};
use task_source_github::transport::ReqwestTransport;
...
impl TransportFactory for ReqwestFactory {
    ...
    fn build(&self, endpoint: &str, token: &str, max_retries: u32) -> Self::Transport {
        ReqwestTransport::new(endpoint, token, max_retries)
    }
}
