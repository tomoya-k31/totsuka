...
///
/// The single-pass property is load-bearing. `{repo_slug}` and
/// `{issue_number}` come from the GitHub API, but the instruction text they are
/// substituted into is rendered next to issue titles and bodies — **content
/// anyone with write access to the repository controls**. A second pass would
/// turn a `{placeholder}` written inside an issue into a directive. Do not
/// reach for a `loop { replace }`.
///
