...
- **マイグレーションを適用するのは `totsuka run` だけ**。`status` / `task` / `focus` / `doctor` は適用しない（スキーマ変更を `run.lock` 下に限定するため）。`task cancel` / `retry` のようにデータを書き換えるコマンドも、ロックを持たない以上ここに含まれる。
...
**DB のほうが新しい**（＝ totsuka をダウングレードした、または新しい版で一度起動した state_dir を古い版で開いた）。`run` でも他のコマンドでも止まる。
...
これは `run` 以外のコマンドでのみ出る。黙って古いスキーマを読むより、適用契機を明示するほうを選んでいる。
