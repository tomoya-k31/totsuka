...
protocol_version = ">=0.6.0, <0.8"  # 対応する Orchestrator プロトコルの範囲
...
**上限**は、下回っていたい破壊的変更の**次**のメジャー／マイナーに置く（現行なら `<0.8`）。上限 `<0.3` のマニフェストは 0.3.0 の Orchestrator に、`<0.4` は 0.4.0 に、`<0.5` は 0.5.0 に、`<0.6` は 0.6.0 に、`<0.7` は 0.7.0 に、それぞれ起動を拒否される。
...
0.7.0 が同じリリースの反例になっている。ワークフローが「ソースの持つ domain のうちどれを走査するか」を伝えるフィールドが入り、複数の domain を持つ github / notion の 2 本だけが `>=0.7.0` へ動いた —— このフィールドより前のビルドは無視して**自分の全ボード**を走査するので、運用者が書いた絞り込みが黙って効かない。domain が 1 つのソースは絞り込みが恒等で、agent と notifier はそもそも読まないので、残る 5 本は `>=0.6.0` のままで上限だけ広げた。
...
JSON-RPC の行処理（パースエラー、notification への無応答、`shutdown`、未知メソッド、params の型検査）は自分で書かなくてよい。`plugin-sdk` の型付きハンドラを実装すれば、残りは SDK が受け持つ。
...
| 置き場所 | 所有の決め方 | 実装すること |
|---|---|---|
| `[[workflows]]` | **聞いて決める。** 余ったキーはその workflow のタスクソース（`projects` の所有者）と `agent` に届き、ちょうど 1 つが引き取る | `claimed_options` に `{workflow, key}` を返す。**消費しないキーを claim しない** —— タイポを沈黙に変える |
...
書いたプラグインがプロトコルの約束事を守っているかは、`plugin-conformance` クレートで確かめられる。キットはプラグインの**バイナリ**を起動して stdio で話すので、`plugin-sdk` を使っているかどうかに関係なく同じ形で使える。公式の 7 本も、それぞれの `tests/conformance.rs` でこのキットを通している。
...
| # | 対象 | 約束事 |
|---|---|---|
| 1 | 全 kind | initialize 前に届いた kind 固有のリクエストは `INVALID_REQUEST`（-32600）で拒否する。対象は、ホストがそのプラグインへ送りうるもの（capability で絞ったもの）。notifier は、initialize 前の `notify` に応答しない |
