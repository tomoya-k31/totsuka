...
形には 3 つ目の状態 `⚠` がある。totsuka は動いているが、仕事を全部はできていない状態である。毎サイクル確認し直せる 4 つを見る —— hook 受信が bind できなかった（その run では完了検知が効かない）、プラグインが落ちている、hook のシグナルが spool に溜まっている、LLM ゲートウェイが API キーを拒否した。いずれも直れば自動的に消える。`totsuka status` の `degraded:` にも同じ理由が出る。
