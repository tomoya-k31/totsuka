...
<!-- generated-from: ai-docs/product/orchestrator-spec.ja.md sha256:e2c036b0e1dfdc22adb0051e5189bdc498513493a4cf560f9bf85a222213acc9 -->
...
形には 3 つ目の状態 `⚠` がある。totsuka は動いているが、仕事を全部はできていない状態である。毎サイクル確認し直せる 4 つを見る —— hook 受信が bind できなかった（その run では完了検知が効かない）、プラグインが落ちている、hook のシグナルが spool に溜まっている、LLM ゲートウェイが API キーを拒否した。いずれも直れば自動的に消える。totsuka が自分では報告できない 5 つ目の場合もある —— プロセスは生きたまま 2 分間まったく報告しなくなったときで、これも `⚠` になる。ハングした run は「自分はハングした」と言えないためである。`totsuka status` の `degraded:` にも同じ理由が出る。

既定の出力は SwiftBar のプラグイン書式で、`--json` は同じ内容をデータとして返す。常に exit 0 で終わり（非ゼロ終了するとメニュー項目ごと壊れるため）、失敗は行として出る。タスクのタイトルは起票した人が書いた文字列なので、SwiftBar へ渡す前に totsuka がエスケープする —— タイトルに `|` や改行があっても、行にパラメータを足したり行を割ったりはできない。設定はシェル 2 行で、運用ガイドに手順がある。
