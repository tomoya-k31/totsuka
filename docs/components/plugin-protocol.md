...
プロトコルは独立した SemVer（`PROTOCOL_VERSION`）を持つ。プラグインは manifest で対応範囲を semver 要件として宣言し、Orchestrator は範囲外のプラグインを拒否する（破壊的変更でメジャーを上げ旧世代を 1 世代サポート）。0.x 世代では caret 意味論（`^0.1` = `<0.2.0`）に合わせ、**後方互換の追加はパッチレベル**で刻む（例: 0.1.1 の `InitializeParams.repositories`。0.2 に上げると全プラグインの `^0.1` manifest が採用不要の変更で弾かれてしまうため）。追加を前提とするプラグインは `>=0.1.1, <0.2` のように宣言できる。
