...
generated: { by: human:tomoya-k31, at: 2026-08-09T11:30:00Z }
status: stable
owner: tomoya-k31
---
...
プロトコルは独立した SemVer（`PROTOCOL_VERSION`）を持つ。プラグインは manifest で対応範囲を semver 要件として宣言し、Orchestrator は範囲外のプラグインを拒否する（破壊的変更でメジャーを上げ旧世代を 1 世代サポート）。0.1 系では caret 意味論（`^0.1` = `<0.2.0`）に合わせ、**後方互換の追加はパッチレベル**で刻んでいた（例: 0.1.1 の `InitializeParams.repositories`、0.1.2 の `InitializeParams.llm`、0.1.3 のフック起動・resume・診断スナップショット一式（#132）、0.1.4 の `session/focus`（#155）、0.1.5 の `Task.instructions`、0.1.6 の `task/submit` push 取り込み（#183））。**0.2.0（#190、[ADR-0008](/decisions/adr-0008-task-submit-push-ingestion.md) Phase C）は `tasks/fetch` 削除の破壊的変更**として実施済み — `^0.1` を宣言する manifest は F-54 により起動拒否される（0.1.6→0.2.0 が猶予窓、意図された動作）。push-only プラグインは `>=0.1.6, <0.3` を宣言することで削除をまたいで動作する（同梱 3 プラグインは全て対応済み。herdr/orca/notifier-macos は tasks/fetch を使っていなかったため `>=0.1.0, <0.3` へ広げるだけで済んだ）。**0.2.1（#210）は 0.2 系初の追加的変更**（`session/release`）で、全同梱 manifest の上限 `<0.3` にそのまま収まるため manifest 変更は不要。**0.2.2（#211）も同様の追加的変更**（`session/list`）で manifest 変更不要。**0.2.3（#196）も追加的変更**（`TaskDispatchParams.tool_launch` + `ToolLaunchSpec`、`hook` の deprecated 化）で manifest 変更不要。**0.2.4（#242、[ADR-0015](/decisions/adr-0015-conversation-task-identity.md)）も追加的変更**（`Task.message_key`、[`task/lookup`](/apis/task-lookup.md) RPC、`SESSION_UNRESUMABLE`）で manifest 変更不要。**0.3.0（#264）は 0.2 系初の破壊的変更** — `Task.thread_key` の削除。ワイヤは壊れない（`Task` に `deny_unknown_fields` が無いため、送ってくる旧プラグインは受理され値が無視される）が、**その値を読むプラグインはコンパイルが通らず**、`task/dispatch` で受け取っていたプラグインには届かなくなる。型の破壊であり第三者に自力で気づかせる筋合いではないので、バージョンで明示する: `<0.3` 上限の manifest は F-54 で起動拒否される（0.2.0 が `^0.1` を切ったのと同じ扱い）。同梱 6 プラグインは `<0.4` へ引き上げ済み。`Task.id` の**意味**は変わるがワイヤ形式は不変で、`message_key` 不在＝従来どおりという既定を置いているため1 メッセージ = 1 タスクのソースは無改修。**0.4.0（#411、[ADR-0034](/decisions/adr-0034-protocol-0-4-0-removals.md)）は 2 回目の破壊的変更** — 0.3.0 が「次の breaking で削除」と宣言しながら落とし忘れた 3 面（`TaskDispatchParams.hook` / `HookLaunchSpec`、herdr の `agent_command`・`plan_args`・`launch_command`、`design_preview` の設定キーとケイパビリティ宣言）の削除。同梱 6 プラグインは `<0.5` へ。**下限は一律ではない**: agent_ide の herdr だけ `>=0.2.3`（`tool_launch` が入った版）まで上げ、argv 自前組み立てのフォールバックを**到達不能**にしてから消した。orca は `tool_launch` を読まないので `>=0.1.0` のまま、task_source は `>=0.1.6` のままである。
