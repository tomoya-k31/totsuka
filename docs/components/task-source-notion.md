...
Notion データベースを totsuka のタスクソースとして接続する公式プラグイン（F-02/F-03）。[plugin-protocol](/components/plugin-protocol.md) を実装する単体バイナリで、stdio JSON-RPC 2.0（NDJSON）サーバとして起動する。[task-source-github](/components/task-source-github.md) と同じ構造を Notion REST API へ適用したもの。
...
`tasks/fetch` は、まずトリガー（`status` / raw `filter`）で候補を絞り（可能なら databases query の server-side filter で削減）、次に多人数運用ゲーティングを適用する: assignee（people プロパティ）が他者のタスクを除外（自分は `notion_user_id` で判定、未設定時は未 assign のみ取り込み）、`in_progress_statuses` のステータスを除外。厳密な排他制御はしない。

# capabilities（F-83）

manifest（`plugins/task-source-notion/plugin.toml`）と `initialize` 応答で `kind = task_source`・`outputs = ["source"]` を宣言。`result/publish` に対応する。

# テスト

`NotionTransport` を録画レスポンスの fake に差し替え、initialize→fetch→property_map 正規化→ページ本文取得→update_status→result/publish の全経路を JSON-RPC 境界越しに結合テスト（`tests/integration.rs`）。取り込み制御（他者 assignee / 実行中 / トリガー不一致）、2000 文字超の publish 分割、未知 option の update_status 拒否、トークン無効／マップ先プロパティ欠落時の `config/validate`（原因＋次アクション）を検証。実バイナリを stdio で駆動して疎通確認済み。

# 依存

- `plugin-protocol`（プラグイン境界）、`reqwest`（REST）、`tokio`、`serde` / `serde_json` / `semver` / `thiserror` / `tracing`。GitHub プラグインと同一の依存集合（新規外部クレートなし）。
