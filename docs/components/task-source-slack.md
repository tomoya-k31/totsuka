...
| `server` | JSON-RPC ディスパッチ `Server<F: TransportFactory>`。initialize（TokenGuard → Socket Mode + パイプラインの常駐ランタイム起動。`without_runtime()` はプロトコルテスト用）/ config·validate（**静的検証のみ・ネットワーク不要**）/ shutdown / tasks·fetch（バッファ全排出、trigger は v1 未解釈、2 回目は同一 Task を返さない）/ task·update_status（意図的 no-op）/ result·publish（`approval::publish_draft` へ委譲。pending 不在・空 content のみ `INTERNAL_ERROR`）。未初期化メソッドは拒否。Session drop で常駐タスクを abort |
