...
| `dispatch` | `Reply` / `request_id` / `parse_params` と、型付き **`TaskSourceHandler`** trait（initialize / config_validate / update_status / result_publish。`tasks_fetch` は deprecated のため既定で空応答）。`TaskSourceServer` が trait を `LineHandler` に変換し、PARSE_ERROR・notification 無応答・shutdown・METHOD_NOT_FOUND を含む wire protocol 全体を実装 |
...
| `poll` | **`poll_loop`**: `InitializeParams.triggers` × `poll_interval_secs` の fetch→submit タイマー（github/notion の `tasks/fetch` 移行用）。tick は非重複、間隔は ±10% jitter（SplitMix64、rand 依存なし）。fetch 失敗はその tick のみスキップ。dedup は Orchestrator 側 `duplicate` ack に委譲し seen-set を持たない |
