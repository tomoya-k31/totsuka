...
- `setup [--dry-run] [--yes] [--save-answers <file>]`（#348、[ADR-0028](/decisions/adr-0028-setup-wizard.md)）: 対話で config.toml を埋める。**interview（純粋・何も書かない）→ 計画印字 → 1 回だけ確認 → apply（各ステップ冪等）** の 2 フェーズで、質問中の Ctrl-C は痕跡を残さず、apply の途中失敗はどこまで適用したかを印字する（原子性ではなく**収束性**を保証 — 再実行で揃う）。config.toml は `.new` へ書いて `rename`。
  ...
  - **非対話は hidden `--answers <file>` のみ**。TTY でなく `--answers` も無ければ **exit 2**（パイプから既定値を黙って採用しない）。hidden にしているのは、これが再現性のための機能ではなく**テスト可能性のための経路**だから — CLI の E2E は端末を持たない子プロセスなので、これが無いと「書いた config が実際に読み込める」を CI で固定できない。
