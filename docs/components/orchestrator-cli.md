...
- `setup [--dry-run] [--yes] [--save-answers <file>]`（#348/#349、[ADR-0028](/decisions/adr-0028-setup-wizard.md)）: 対話で config.toml を埋める。**interview（純粋・何も書かない）→ 計画印字 → 1 回だけ確認 → apply（各ステップ冪等）** の 2 フェーズで、質問中の Ctrl-C は痕跡を残さず、apply の途中失敗はどこまで適用したかを印字する（原子性ではなく**収束性**を保証 — 再実行で揃う）。config.toml は `.new` へ書いて `rename`。
  ...
  - **既存ファイルはスキップ**（上書きも差分マージもしない）。**唯一の例外が `init` の吐いた「全行コメント」の雛形**で、これを未設定扱いにしないと、ドキュメントどおり `init` を先に打った人は `setup` が永久に何もしない。
  - **機密の値を一切扱わない**。バックエンド（Keychain / 1Password / env）を一度だけ選ばせ、参照名は規約値で自動生成し、登録コマンドのチェックリストを印字する。実体の検証は `doctor` に委ねる。回答ファイルがトークンを含みえないのはこの帰結（`deny_unknown_fields` が強制）。
  - **非対話は hidden `--answers <file>` のみ**。TTY でなく `--answers` も無ければ **exit 2**（パイプから既定値を黙って採用しない）。hidden にしているのは、これが再現性のための機能ではなく**テスト可能性のための経路**だから — CLI の E2E は端末を持たない子プロセスなので、これが無いと「書いた config が実際に読み込める」を CI で固定できない。回答ファイルは**レシピが宣言する穴（`github` / `slack_user_id` / `llm`）が埋まっているかも検証する** — 対話経路は穴をそのまま質問するので空きえないが、手書きのファイルは空きうる。素通ししてしまうと `verification = "llm"` かつ `[llm]` 不在の config が生成され、setup は成功と報告して実行時に落ちる。
  - **#349: config を書いて終わりではない**。同一コマンドで続けて (a) `plugins/<name>.toml` を生成し、(b) レシピのプラグインを install + enable し、(c) `doctor` をインプロセス実行して **exit 3 を伝播**する。新マシンで打つコマンドが 1 本になるのがこの epic の目的なので、途中で「次はこれを実行」と案内して終わるのは目的を果たさない。
    ...
    - **`plugins/<name>.toml` は「既定値を持たない必須キー」だけを書く**。`herdr` / `macos` は全フィールドに既定があるのでファイル自体を作らない（空ファイルは「人が書いて空にした」と区別できず、以後 setup が埋め直さなくなる）。`github` は `owner` / `owner_type` / `project_number` / `github_login` が、`slack` は `app_token` / `user_token` / `target_user_id` が必須で、これが `Blank` として質問になる根拠。**例外は `slack` の `bot_token` 1 つだけ**で、プラグイン的には opt-in（無ければナッジ無し）だが、本人名義の返信は Slack 通知を一切上げないためナッジが無いとフロー全体が気づかれない（[ADR-0021](/decisions/adr-0021-slack-bot-notification-nudge.md)）。レシピが書く以上、未登録だと「ナッジだけ無効」ではなく**プラグインの起動自体が失敗する**ので、シークレットのチェックリストに載せ、各項目が何を可能にするかを併記する。トークンは参照のみ（値は通さない）。生成物は**プラグイン自身のデシリアライザ**（`GithubConfig` / `SlackConfig`）が受理することをテストで固定する — 「TOML としてパースできる」では、キー名を 1 つ間違えても実行時まで露見しない。
    - **既存の `plugins/<name>.toml` はスキップ**（config.toml と同じ規則。ただし雛形の例外は無い — これを生成する `init` 相当が存在しないため、在るファイルは人か過去の setup が書いたもの）。
