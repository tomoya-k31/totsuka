...
- 設定スキーマはバージョンキーを持つ。不一致は起動時検証(`totsuka config validate`、および `totsuka run` の同じ検査)がエラーにするだけで、設定を自動で書き換えることはない。スキーマは v1 のままで移行すべき差分が無いため、マイグレーションは提供しない。v2 を切る前に決めるべきことは [設定リファレンス](/development/config-reference.md) を参照（日本語版のみ）。
