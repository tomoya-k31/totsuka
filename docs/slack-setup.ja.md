...
会話に見える投稿はすべてユーザートークンで行われる。アプリの bot ユーザーは通知 DM を送るためだけに存在する — エフェメラルメッセージは、それ自体では Slack の通知を発生させないためである。
...
| 長く止めたとき | **Slack が購読を無効化する**（60 分の配信試行の 95% 超が失敗したアプリ）。復旧は Slack の設定画面での手作業で、それが起きたことを totsuka に知らせるものは何も無い | **totsuka が止まっていることは、もう原因にならない。** ゲートウェイ自身が落ちる・URL が誤っている・publish が失敗する、では配信は失敗するので「常に成功する」ではない |
...
4. このページの手順 2（トークン保管）と手順 3（`totsuka setup` と設定の編集）を通す
5. **`setup` が書いた `[slack]` テーブルに `event_source` と `[slack.gateway]` を足す**

**手順 5 を手順 4 に畳めない理由。** `setup` は**既に存在する `[slack]` テーブルには触らない**ので、
先に自分で書いてしまうと `user_token` と `target_user_id` が永久に入らない。かといって `setup` に
任せると Socket Mode 前提の `app_token` が書かれ、`setup` の最後に走る `doctor` が App-Level Token を
要求して落ちる。**「先に全部書く」順序はどちらも成立しない** —— だから `setup` に書かせてから足す。
`setup` 中の `doctor` が赤いのは想定内で、手順 5 のあとに `totsuka doctor` を回し直せばよい。
...
聞かれるのは使うプラグインだけである（`--plugins` を渡さなければチェックボックスで選ぶ）。**トークンの値は一切聞かれない。** **totsuka が解釈する設定を全部コメントで書いた `config.toml`** が生成され、そのパスが印字される。

そのファイルを開いて、少なくとも次のコメントを外す。`[slack]` の節はすぐ見つかり、各キーに 1 行の説明が付いている。

1. `[[repositories]]` —— リポジトリのパスと名前
2. `[plugins.slack] enabled = true` と `[plugins.herdr] enabled = true`
3. `[slack]` の `app_token` / `user_token` / `bot_token` / `target_user_id`（手順 1 で控えたメンバー ID）
4. `[[projects]]` の slack エントリと `[[workflows]]` —— **ファイル末尾のレシピ集にある「Slack — reply as yourself」**をそのままコメント解除するのが早い

トークンをまだ保管していなければ、`setup` が実行すべきコマンドを既に印字している。
...
**`state-db` チェックだけは fail のままで `doctor` は exit 3 で終わる。** これは状態データベースがまだ無いというだけで、作るのは `totsuka run` だけである。1 回走らせれば緑になる。

### 設定を手で書く場合

`setup` は**既存ファイルの行を書き換えない**（足りない節を追記するだけである）ので、生成された雛形を使わず自分で書いてもよい。プラグインだけ入れて設定は手で書く形:
...
通しで試すには、誰かに自分宛のメンションをしてもらう。エージェントの完了後、スレッド内のエフェメラルメッセージに返信案が届く（ボタンがあるのはここだけ。`bot_token` を設定していれば bot からの通知 DM も届く）。**承認**すると本人名義のスレッド返信として投稿され、**却下**すると破棄される。どちらの場合も押した時点でエフェメラルは消え、✅ / ❌ は代わりに bot からの通知 DM に書き込まれる —— 消えない場所に決着が残る。`bot_token` を設定していない場合は書き込む先が無いので、従来どおりエフェメラルがその場で ✅ / ❌ に書き換わって残る。
...
| 返信案は届くがボタンが効かない | 24 時間で失効する。または下書きが 1024 件を超えて追い出された。`bot_token` を設定していれば通知 DM に返信案の本文が残っているので、そこから手で返信するか、もう一度メンションする。下書きは再起動しても残る |
...
| **Gateway**: 復帰しても何も起票されない | イベントが `drain_max_age_hours`（既定 24）より古い。キューは 7 日保持しているので設定を上げれば拾えるが、**上げるのは不在明けの最初の起動より前**であること。窓の外と判定したイベントはその場で ack して捨てるので、**起動した後に上げても捨てたぶんは戻らない** |
| **Gateway**: 監視チャンネルの反応が遅い | 仕様。Gateway ではチャンネル履歴のポーリング（`watch_poll_interval_secs`、既定 60 秒）で監視し、メンション・リアクション・ボタンはキューから来る。ただしそちらも秒数の保証ではない —— キューの読み取りは待つことが許されているだけで保証はなく、空応答が続けば読み取り側はバックオフする。ゲートウェイを通るのは自分を名指ししたものだけで、監視チャンネルへの普通の投稿は該当しない |
