...
**したがって記録は毎 cycle 呼ぶ。** DB 側で dedup されるので繰り返しは索引 1 回のルックアップで済み、プロセス内の集合で更に囲うと 2 つ失う: 書き込みが一度失敗したら待機が終わるまで説明が残らないこと、そして `missing` が変わってもノートが古いままになること（この節の最後の段落が要求していることが、その集合の内側では起きない）。
