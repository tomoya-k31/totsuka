...
<!-- generated-from: ai-docs/development/config-reference.md sha256:554d720f8016293c27d80589c998ab70f2c4f69a871bb6261ac980ed5e0b5d25 -->
...
タスクソースが持つ、名前で指せる管轄単位。GitHub Project のボード、Notion のデータベース、Slack のワークスペース、Discord のギルド。

エントリは 2 方向から指される。ワークフローの `projects` は**タスクを引く先**の domain を、リポジトリの `project` は**起票する先**のトラッカーを名指す。GitHub と Notion では両者が一致するが、別の関係である。

**domain が 1 つしか無いソースもエントリが要る。** Slack と Discord は 1 つずつなので下の 2 キーだけになるが、省略はできない —— ワークフローはプラグインではなく domain を指すためである。

| キー | 型 | 既定 | 意味 |
|---|---|---|---|
| `name` | string | 必須 | `[[repositories]].project` と `[[workflows]].projects` が指す安定した ID |
| `source` | string | 必須 | この domain を所有する task_source プラグイン |
...
- **`mention = true` のワークフローが 2 つあるのもエラー。** メンションが先に書いたほうへ吸われる
- **順序は関係ない。** メンションとリアクションはプラグイン内で別の経路を通るので、リアクションのワークフローをメンションの後ろに書いても隠れない
- 起動するのは自分が付けたリアクションだけで、これを緩める設定は無い

#### `from_bot` — その絵文字を bot の投稿にも効かせる

既定では、リアクションでタスクにできるのは**人間の投稿だけ**である。bot が流す通知を起点にしたい場合は、その bot の id をトリガーに並べる。
...
- **起動するのは相変わらず自分が付けたリアクションだけである。** 許可した bot が投稿しただけでは何も起きない。広がるのは「何に向けて付けられるか」だけ
- 書くのは投稿に付く `bot_id`（`B…`）で、アプリ名でもチャンネル名でもない。チャンネル単位の許可にしていないのは、それだとそのチャンネルに居る bot 全部を許すことになるため
...
**組み込みの既定文は英語で、言語を名指ししない。** 成果物の言語は、エージェントが従う規則で決まる —— スレッド / 元 issue / 元ページと同じ言語で書く、という規則である。自分で上書きするときも**言語名は書かないほうがよい**: 書くと、エージェント側の設定と元メッセージの言語の両方を上書きすることになる。特定の言語を強制したいときだけ明示する。

タスク**本文**のラベル（Slack ソースの `body_template` と添付・スレッド文脈のキー）は別の判断で、日本語のまま残してある。ペインでそれを読むのは人間で、上の指示文はエージェントしか読まないためである。
...
| `source_name` | string | `github` | 各タスクに刻印されるソース名。ボードを増やしても変わらない —— タスクの `source` はプラグインの識別子で、どのボード由来かは持たない。ワークフローの側は `projects` でボードを個別に名指す |
...
| キー | 型 | 既定 | 意味 |
|---|---|---|---|
| `app_token` | string? | `socket` では必須 | ソケット接続用の App-Level Token（`xapp-`）。シークレット参照を推奨。**`event_source = "gateway"` では不要** —— WebSocket を開かないので用途が無い。省略してよく、置いたままでも `xapp-` 形式の検査だけは効く。`socket` で欠けていると起動しない |
| `user_token` | string | 必須 | 本人名義の読み書きに使う User OAuth Token（`xoxp-`）。シークレット参照を推奨 |
| `bot_token` | string? | なし | Bot Token（`xoxb-`）。設定すると、返信案やピッカーの到着時に bot が本人へ DM を送る。**未設定なら機能が off になるだけ**（起動時に警告 1 回）。ただし**チャンネル監視トリガを使うなら必須**で、未設定だと起動しない —— 監視の結果は bot 名義で投稿するため。bot を監視チャンネルに招待しておくこと（未招待だと投稿時に `not_in_channel`） |
...
| `[slack.gateway]` | テーブル | なし | `event_source = "gateway"` のときのキューの場所。`project` / `subscription`（メッセージ・リアクション）/ `block_actions_subscription`（ボタン押下）/ `pubsub_url`（既定 `https://pubsub.googleapis.com`）/ `pull_max_messages`（既定 50）。**サブスクリプションが 2 本あるのは保持期間が違うから**で、同じ名前を 2 回書くと拒否される —— 押下が日単位の保持に載り、2 つの取り込みループが同じメッセージを取り合うことになるためである。認証は `gcloud auth application-default print-access-token` へのシェルアウトなので、**サービスアカウントキーを配らない** —— 各利用者が自分の Google アカウントで引き、自分のサブスクリプションだけを許可される |
...
| `body_attachment_header` | 添付ファイルセクションの見出し。メッセージが添付を持つときだけ出る。**既定文は 2 つのことを言っており、どちらも振る舞いである**: ①「the content was not handed over」—— このプラグインは `files:read` を持たないので中身をダウンロードしない。言わないとエージェントが内容を推測して返信する。文中で指してよいのは、そのプロンプトに実際に書かれているものだけである —— エージェントが読むのは指示文と本文だけで、システム名も「タスク」という作業単位もそこからは見えない。②「where a line carries a link, fetch the file from it yourself」—— エージェント自身の Slack ツール（MCP 等）は permalink から file id を切り出して中身を読める。条件形なのは permalink が省かれる添付があるためで、無条件に書くと存在しないリンクを指すことになる。上書きするなら**両方**残すこと。既定文がツール名を出さないのは意図的で、名指しすると持っていないエージェントに存在しない道具を探させる。**このキーだけ見出しと括弧内で言語が違う**のも意図的で、見出しはペインを見る人間が読むラベル（利用者の言語）、括弧内はエージェントへの指示（英語）である | `{count}` |
...
| `body_thread_permalink` | **親スレッドの permalink**。メンションがスレッド内の返信だったときだけ出る。トップレベルのメンションはスレッドの根そのものなので、その permalink はタスクの URL が既に持っており、セクションは出さない。**リアクション由来のタスクがスレッドの根ではなく 1 つの返信に付いた場合も出さない** —— そこではスレッド文脈を意図的に渡していない（エージェントは 1 通のメッセージを指されたのであって、会話を指されたのではない）ため、入口だけ渡すと本文を渡さないと決めた判断を裏口から戻すことになる。スレッド文脈と本キーは会話についての 1 つの言明であって、スコープ規則を共有する。**括弧内の文がこのキーの本体である**: タスクの URL はメンション自身を指すので、スレッドの途中でメンションされたエージェントには会話の入口が渡らない。本文に入るスレッド文脈は直近 `thread_context_limit` 件の**窓**であって、その外側へ辿る手段ではない。既定文は「文脈は窓である／この URL がその外側への入口である」と明言しており、落とすと切り取られた会話をそのまま全体だと思って答えるエージェントに戻る。「any thread context below」と条件形で書いてあるのは、下のセクションが常に本文とは限らないため —— 文脈の取得に失敗すると `body_thread_unavailable` に差し替わり、そこではこのリンクの価値はむしろ上がる。`body_attachment_header` と同じく**見出しは利用者の言語・括弧内は英語**で、前者はペインを見る人間が読むラベル、後者はエージェントへの指示だからである | `{url}` |
