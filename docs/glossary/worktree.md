...
`git worktree` で作られるタスク専用の作業ディレクトリ（F-20〜25）。「1 task = 1 repo = 1 worktree = 1 branch」を不変条件とし、ブランチ名は**エージェントが命名する** — worktree は detached HEAD で引き渡され、エージェントがリポジトリの命名規約（`<type>/<slug>` 等）を読んで `git switch -c` する。Orchestrator は `HEAD` を読み戻して名前を知る。**配置先のディレクトリ名はブランチ名からは作らない** — 既定は `{source}-{task_id}` を git ref 規則で正規化した `{worktree_name}`（Slack なら `slack-C0ABCDEF12-1720000000.123456`）で、worktree を作る時点でブランチ名がまだ確定していない場合にも使えるようにするため。タスク完了・キャンセル後は掃除ポリシー（`immediate` / `{ retention_days = N }` / その糖衣 `keep_7d`・`keep_28d` / `manual`、F-23/85）が適用されるが、未コミット変更があるものは決して削除しない。どのタスクにも属さないものは孤児（orphan）として `totsuka doctor` が検出・掃除を提案する（F-24）。
