...
2. **herdr 内 pane フォーカス** — クリックの `-execute` が `totsuka focus <task_id>` を実行し、制御 UDS [`POST /focus`](/apis/claude-events.md) → Engine が task→session を解決 → agent_ide プラグインの `session/focus`（プロトコル 0.1.4、`pane_control` 宣言時のみ）→ herdr の `workspace.focus`→`tab.focus`→`pane.focus` チェーン。session id の復号はプラグイン内に閉じる（F-37 不透明契約）。
