...
| LLM ゲートウェイが答えない | 到達できない・タイムアウトした・サーバエラーを返した、のいずれかで、行にどれかが書いてある。分類が要るタスクは戻るまで失敗する。ネットワークと `[llm].base_url` を確認するか、`totsuka doctor --online` を叩く（同じリクエストを送るので、両者の「生きている」は一致する）。totsuka はこれを自分で確かめている —— 起動時に 1 回、Mac がスリープから復帰した直後、ゲートウェイへの呼び出しが 10 分間なかったとき（落ちている間は 1 分おき）。ゲートウェイが何か答えを返した時点で、401 であっても行は消える |
