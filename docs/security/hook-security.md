...
- **opencode の plan エージェントは frontmatter と散文を分離している**（#316）。`agents/totsuka-plan.md` は Rust 側固定の YAML frontmatter（`mode: primary` と `permission: {edit: deny, bash: deny, task: deny}`）と、`[prompts].opencode_plan_agent` 由来の散文本体を連結して書き出す。**frontmatter は設定不可** — この deny マップが plan モードの読み取り専用保証そのもので、散文に見えるキーから `bash: allow` を注入できると権限昇格になるためである。opencode には claude の `--permission-mode plan` や codex の `--sandbox read-only` に相当する構造的な plan フラグが無く、**このファイルが plan 意図の唯一の強制手段**なので、ここが崩れると plan モードが名目だけになる。散文が `---` の行を**どこかに**含む場合は検証エラーで弾く（`permission:` ブロック密輸のガード）。先頭だけでなく全行を見るのは、**opencode のパーサをこちらで検証できない**ためである — 慣例上 frontmatter はファイル先頭でしか解釈されず後続の `---` はただの水平線のはずだが、権限境界の保証を第三者パーサの挙動に関する推論の上に置きたくない。本文の水平線は `***` で書ける。JS プラグイン側は従来どおり全体が埋め込みで設定不可である。
- **claude の plan 系 profile は `--settings` に `permissions.deny` を持つ（#395、[ADR-0033](/decisions/adr-0033-workflow-profile.md) D4）。** これも Rust 固定で、設定キーからは合成できない — 理由は上の 2 項と同じで、散文に見えるテキストから権限を配れる状態を作らないため。効く根拠は Claude Code の permission モデル自体にある: **deny はスコープ横断でマージされ、どこかで deny されたツールは他のどのスコープの allow でも許可できない**。したがって totsuka が `--settings` で渡す deny は、対象リポジトリの `.claude/settings.json` の allow に必ず勝つ。公式ドキュメントが「Permission rules are enforced by Claude Code, not by the model. Instructions in your prompt or CLAUDE.md ... don't change what Claude Code allows.」と明言しており、これが [#378](https://github.com/tomoya-k31/totsuka/issues/378)（リポジトリの `CLAUDE.md` に誘導されて plan タスクが push・PR まで到達した）への直接の答えになる。**保証の強さは層で違う**: 裸のツール名（`Edit` / `Write` / `NotebookEdit`）はツールごと除去されるのでファイル編集の実質保証になるが、`Bash(...)` はコマンド文字列への前方一致でしかなく、`/usr/bin/git push` やチェーン内の実行は素通りする — 事故低減であって境界ではない。ハード保証には sandbox が要る（未着手）。なお **`Write(path)` / `NotebookEdit(path)` のパス付きルールは受理されるが参照されない**ため、裸名のみを使っている。
