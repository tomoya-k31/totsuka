...
- **opencode の plan エージェントは frontmatter と散文を分離している**（#316）。`agents/totsuka-plan.md` は Rust 側固定の YAML frontmatter（`mode: primary` と `permission: {edit: deny, bash: deny, task: deny}`）と、`[prompts].opencode_plan_agent` 由来の散文本体を連結して書き出す。**frontmatter は設定不可** — この deny マップが plan モードの読み取り専用保証そのもので、散文に見えるキーから `bash: allow` を注入できると権限昇格になるためである。opencode には claude の `--permission-mode plan` や codex の `--sandbox read-only` に相当する構造的な plan フラグが無く、**このファイルが plan 意図の唯一の強制手段**なので、ここが崩れると plan モードが名目だけになる。散文が `---` で始まる場合は検証エラーで弾く（frontmatter の二重化を防ぐと同時に、`permission:` ブロック密輸のガードを兼ねる）。JS プラグイン側は従来どおり全体が埋め込みで設定不可である。
