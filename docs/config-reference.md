...
- `totsuka doctor` and `totsuka config validate` accept `--secrets-stdin` too: given the same line, they check your setup with the real values and still open no secret store. Without it they do not resolve `secret:` references — they note them and skip the plugin checks that would need them, without reporting a failure.
...
| `plan_args` | string[]? | per kind | Extra arguments in plan mode. claude: `["--permission-mode", "plan"]`; codex: `["--sandbox", "read-only", "--ask-for-approval", "never"]`; opencode: `["--agent", "totsuka-plan", "--auto"]`. For opencode, `--standalone` is always added first, whatever `mode_args` / `plan_args` say: without it the session runs in opencode's shared background server, which cannot see the task's environment, so completion is never reported. Requires opencode v2 |
