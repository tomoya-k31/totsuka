...
| `body_thread_permalink` | **The parent thread's permalink**, emitted only when the mention is a reply inside a thread. A top-level mention is the thread root, so its own link is already the task's URL and the section is omitted rather than repeating it. **The parenthetical is the point of the key**: the task's URL names the mention itself, so an agent mentioned partway down a thread has no way back to where the conversation started. The thread context in the body is a *window* — the most recent `thread_context_limit` messages — not a handle on what falls outside it. The default text says both of those out loud; drop it and the agent answers from a truncated conversation as if it were the whole one. Like `body_attachment_header`, **the heading is in your language and the parenthetical is in English**, because the heading is a label a human reads in the pane and the parenthetical addresses the agent | `{url}` |
