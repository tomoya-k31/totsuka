...
6. **複数トラッカー（S7、#542）** — 設定形式が変わったとき。まず 1 ボード構成が
   壊れていないことを確かめてから 2 ボード目を足す。**起票先の注入は「issue が立ったか」
   ではなく「ボードに載ったか」で判定する**（`destination` は機械検査されない散文で、
   守っているのは検収 rubric だけ）
7. **ボード単位のレーン語彙（S8、#626）** — `[[workflows]].projects` に手を入れたとき。
   Status 語彙の違う Project #8 を使う。**3 枚目のカードは cli repo で置く**（web は repo の
   紐づけで落ちるので `projects` の絞り込みを測ったことにならない）
8. **Bitwarden シークレット参照（S9、#699）** — `bw:` に手を入れたとき。**CI に `bw` は
   無く、バックエンドは runner seam のユニットテストで閉じているので、実 `bw` に当たるのは
   ここだけ**である。核心は `BW_SESSION` 無しで**ハングせず即エラーになる**ことで、
   ここが効いていないと常駐運用が無言で止まる
9. **decisions モデルでの分類（S10、#723）** — `[llm].api = "decisions"` や
   `repo-classifier` の decisions 実装に手を入れたとき。**API 単体の実キー検査を最初に回す**
   （Slack 不要・数秒・数セント未満）。OpenRouter の Decisions API は **alpha** なので、
   404 や形の変化はまずここで出る
