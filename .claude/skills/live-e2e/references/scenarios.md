...
## S9. Bitwarden シークレット参照（#699 / ADR-0076）🙋

**`bw:` を実装したら必ず回す。** CI には `bw` が無く、バックエンドは runner seam の
ユニットテストで閉じているので、**実 `bw` に当たるのはここだけ**である。
...
- **ADR-0069 §8 のメモ無し経路（#626）** — 再起動でメモが消えた状態のフォールバック探索が `projects` に閉じること。S8 は fetch 時のメモが有効なまま完了したので、取り違えの条件（再起動 → メモ消失 → 設定順が不利）は結合テストまで
- **Notion の `projects` 絞り込みと option 実在検査（#626 / #628）** — 上と同じ理由で 1 度も回していない
