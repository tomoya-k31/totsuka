...
task-source-slack は **User OAuth Token（`xoxp-`）で本人として** 動く（会話に見える投稿はすべて本人名義。bot user は通知ナッジ DM 専用 — [ADR-0021](/decisions/adr-0021-slack-bot-notification-nudge.md)）。`xoxp-` を持つ者は、付与スコープの範囲で **本人になりすませる**:
...
`[[workflows]].trigger.reaction` を設定すると、**本人が付けたリアクション**がタスクを起こす。この「本人が」は設定ではなくコード上の不変条件で、**緩和する設定は存在しない**。
...
- トークンは **macOS Keychain のみ** に保存し、`[slack]` からは `keychain:<service>/<account>` 参照で解決する（F-64/F-65）。設定ファイル・リポジトリ・環境変数ファイルへの平文書き込みは禁止。
...
1. **止める**: <https://api.slack.com/apps> → 対象アプリ → **OAuth & Permissions → Revoke All Tokens**（`xoxp-` と `xoxb-` を即失効）。`xapp-` は **Basic Information → App-Level Tokens** から該当トークンを削除。
2. **確認**: `totsuka run --watch` / `doctor` が `invalid_auth` / `token_revoked` ガイダンスで失敗することを確認（= 旧トークンが死んでいる）。
3. **再発行**: アプリを再インストールして新しい `xoxp-` / `xoxb-` を取得、`xapp-` を再生成。
4. **差し替え**: Keychain のエントリを更新（`security add-generic-password -U …`。`slack-user` / `slack-bot` の**両方**）→ `totsuka doctor` で TokenGuard（`auth.test` + `apps.connections.open` + bot probe）が green になることを確認。
...
- アプリ導入が承認制の場合、[manifest 雛形](https://github.com/tomoya-k31/totsuka/blob/main/plugins/task-source-slack/manifest.yml) のスコープ一覧で申請する（読み書きは user scopes = 本人の権限範囲のみ・bot は本人への通知 DM 専用の最小スコープ（chat:write / im:write）・Socket Mode で外部公開エンドポイントなし、が説明ポイント）。
- メッセージの読み取り範囲は「本人が参加しているチャンネル」に限られるが、その内容がローカルのタスク本文・ログ（`log_prompts`）・LLM 分類リクエスト（`[slack]` の `[llm]` 先）へ流れることを理解した上で、社外 LLM エンドポイントの利用可否を確認する。
