...
generated: { by: claude-code/opus-5, at: 2026-08-17T07:10:00+09:00 }
status: stable
owner: tomoya-k31
---
...
- **プロンプト文で config 由来なのは `rubric` 1 キーだけ（#314 / [ADR-0023](/decisions/adr-0023-configurable-prompt-surface.md)、[#465](https://github.com/tomoya-k31/totsuka/issues/465) で縮小）**: 上の「静的埋め込み」は**実行されるコード**についての主張であり、いまも成立する — フックスクリプト 6 本と opencode の JS プラグインは 100% 埋め込みで設定不可のままである。#314 は**エージェントに見せる散文**を `[prompts]` / `[[workflows]].prompts` から上書き可能にしたが、#465 がその 2 面を削除し、残るのは `[[workflows]].rubric`（llm 検収の判定基準文）だけになった。`config.toml` はリポジトリではなくユーザー自身の XDG config 配下にあるため信頼境界はどちらの時点でも変わらない（ペインに直接打ち込むのと同じ領域）。**プロンプトキーはスクリプト・argv・permission ブロックを追加も改変もできない**という一線は当時も今も条件である（散文に見えるキーから `bash: allow` を注入できると権限昇格になるため）。ステータスマーカー自体も設定不可（[ADR-0020](/decisions/adr-0020-status-marker-stays.md)）。なお config 由来になっても改ざん検知は嘘にならない: `verify_assets` は毎回**再レンダリングした期待値**を作って `verify_one` に渡すので、ディスク上のファイルへの改変は従来どおり検出される（per-workflow の `orchestrator-<workflow>.json` は初日から `rubric` 経由で config 由来であり、そこは #465 の後も変わらない）。
- **opencode の plan エージェントは frontmatter と散文を分離している**（#316）。`agents/totsuka-plan.md` は Rust 側固定の YAML frontmatter（`mode: primary` と `permission: {edit: deny, bash: deny, task: deny}`）と、組み込みの散文本体を連結して書き出す（散文は `[prompts].opencode_plan_agent` から上書きできたが、#465 でそのキーごと削除され両方が設定不可になった）。**frontmatter は設定不可** — この deny マップが plan 意図を運ぶ**唯一の機構**（保証ではない。read-only はどこでも保証しないと決めており（[ADR-0045](/decisions/adr-0045-read-only-is-not-guaranteed.md)）、opencode 実機での挙動も未計測である）で、散文に見えるキーから `bash: allow` を注入できると権限昇格になるためである。opencode には claude の `--permission-mode plan` や codex の `--sandbox read-only` に相当する構造的な plan フラグが無く、**このファイルが plan 意図の唯一の強制手段**なので、ここが崩れると plan モードが名目だけになる。散文が設定可能だった間は、`---` の行を**どこかに**含む値を検証エラーで弾いていた（`permission:` ブロック密輸のガード）。#465 で散文が組み込み専用に戻ったため、この検査は密輸経路ごと無くなった — 弾く対象だった「運用者が書いた値」が存在しない。組み込みの散文に `---` が無いことは `plan_agent_renders_the_pre_316_bytes` が全バイト一致で固定している。JS プラグイン側は従来どおり全体が埋め込みで設定不可である。
- **claude の plan 系 profile は `--settings` に `permissions.deny` を持つ（#395、[ADR-0033](/decisions/adr-0033-workflow-profile.md) D4）。** これも Rust 固定で、設定キーからは合成できない — 理由は上の 2 項と同じで、散文に見えるテキストから権限を配れる状態を作らないため。deny 自体はスコープ横断でマージされるので、totsuka が `--settings` で渡す deny は対象リポジトリの `.claude/settings.json` の allow に勝つ。**ただしこれは read-only の保証ではない。** [#410](https://github.com/tomoya-k31/totsuka/issues/410) の実機検証で、ルールがすべて正しく生成・適用・発火した状態のまま、`answer` profile のタスクがブランチを切り commit し push し PR まで作った。塞げているのは「その名前のツール」と「その文字列で始まるコマンド」だけで、(a) `Edit`/`Write` を消しても `Bash` の `cat >` / `python3 - <<EOF` でファイルは書ける、(b) 前方一致は `git add -A && git commit` や `git push … | tail` のような連結・パイプに負ける。**ここより前の版では「`CLAUDE.md` が push / PR を指示していても効く」と書いていたが、実機で成立しなかったので撤回した** — 守れない約束が書いてある状態は、何も書いていないより危険である。ハード保証には sandbox が要るが、**実装しないと決めた**（[ADR-0045](/decisions/adr-0045-read-only-is-not-guaranteed.md)）ので**この穴は塞がらないまま残る**。なお `Write(path)` / `NotebookEdit(path)` のパス付きルールは受理されるが参照されないため、裸名のみを使っている。
