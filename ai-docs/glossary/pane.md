...
3 つとも専用フラグを新設せず `pane_control` に相乗りしている（focus も release も list も「pane 表面の制御」で分離する意味がない）。宣言しないプラグイン（mock 等）では単に呼ばれず、Orchestrator は pane 操作をスキップして静かに縮退する。orca は [ADR-0081](/decisions/adr-0081-orca-herdr-parity.md) から宣言しており、orca の端末（タブ）が pane に当たる。orca の端末 handle は再利用されないので、下の同一性ガードは herdr ほどの意味を持たない。
