...
| `git_timeout_secs` | int? | `300` | totsuka が実行する git のコマンド 1 回の上限秒数。超えたら git をプロセスグループごと止め、そのコマンドを失敗させる（dispatch 中なら自動で再キュー）。`0` で上限なし（#764 → [ADR-0092](/decisions/adr-0092-git-timeout.md)） |
