...
- `reaction` を伴わない `from_bot`、および `channel`（監視トリガ）との併記は `initialize` が `CONFIG_INVALID` で弾く。**どちらも有効キーの組み合わせなので未知キー検査では捕まらず**、放置すると「bot を許可したのに無反応」として現れる。空配列 `[]` も同じ理由で拒否する
