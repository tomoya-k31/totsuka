...
| `transport` の `call_typed` / `to_params` | 生成型で送って生成型で受ける `call` のラッパ（#519）。デシリアライズ失敗は `InvalidResponse` にメソッド名を載せて返し、**呼び出しごとに既に決まっている**エラー経路にそのまま乗る（warn だけ／タスク失敗／成功扱い）。**厳しくなるのは削除に対してだけ**で、追加（未知フィールド・未知バリアント）は今までどおり通る |
