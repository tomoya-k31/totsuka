...
- **再送を使い切った後に限る**（#685）。resume 付き dispatch の最初の拒否では発火しない —— 発火するのは `agent.start` を計 4 回撃ってもエージェントが addressable にならなかったときだけである。「pane ごと落ちた」という当初の読みは 2026-09-15 の実測で否定された（失敗した resume はシェルのプロンプトを残す）ので、この条件は「死んだ証拠」ではなく「手を尽くした証拠」として読む
