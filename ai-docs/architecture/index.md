...
<!-- okf:index:begin -->
* [ワークスペース依存境界ルール（Fitness Function）](workspace-dependency-rules.md) - ヘキサゴナル構成の依存不変条件（plugins → plugin-protocol / plugin-sdk / repo-classifier のみ（dev は + test-support / plugin-conformance）、plugin-protocol と repo-classifier は leaf、plugin-conformance は plugin-protocol のみ、依存循環なし、core の domain / ports は config と adapters を参照しない）と、それを CI で機械検証する scripts/arch-lint.sh の仕組み・正当な依存追加時の更新手順。
