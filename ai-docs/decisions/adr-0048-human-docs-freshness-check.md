...
生成物 → ソースだけでは足りない。**検査は「在るもの」しか見ないので、生成物を消した／片方の言語を作り忘れた変更が素通りする。** それはこの機構が塞ぐはずの「忘れる」そのものである。

そこでソース側にも機械可読な宣言を置き、逆方向からも突き合わせる:

```text
<!-- generates: docs/config-reference.md docs/config-reference.ja.md -->
```

検査は、宣言された生成物が実在し、かつ**こちらを名指すマーカーを持っている**ことを確かめる。**対応表の正はこの宣言（ソースファイル自身）**であって、スキルの表はその写しである — 2 箇所に書けば必ず片方が腐る。
...
`EXEMPT` の判定は**マーカーを探す前**に行う。後に置くと、マーカー形式を本文で説明する手書きページが「壊れたマーカーを持つ生成物」に見え、`EXEMPT` に入れても逃げられなくなる。あわせて、マーカーを探す範囲を**先頭 20 行**に限っている（規約上そこに置くため）。

**残る穴を正直に書いておく**: `EXEMPT` に入っていない手書きページが、先頭 20 行以内にマーカー形式の文字列を含むと誤検出される。エラー文が `EXEMPT` に足すよう案内するので行き止まりにはならないが、自動では解けない。

## 検査対象ゼロを成功にしない

`docs/` に `.md` が 1 つも無ければエラーにする。放置すると、ディレクトリを消した変更や引数を誤った CI が **0 error で緑になる** — 「何も検査しなかった」と「すべて正しかった」が区別できない状態は、この ADR が避けようとしているものそのものである。
...
- **対応表をスクリプト側に持つ** — ソース側の宣言と二重管理になり、必ず片方が腐る。宣言を正とした。不採用。
