...
2. FIFO は `${XDG_RUNTIME_DIR:-<state_dir>}/totsuka/orca-env/` に置く。Orchestrator の `runtime_dir` と同じ規則で、hook ソケットの隣になる。ディレクトリは `0700`、FIFO は `mkfifo -m 600` で作る。中身は 1 変数 1 行の `export K='v'` で、値は起動の各語と同じ単一引用符でクォートする。**キーがシェルの識別子でなければディスパッチを拒否する**（`export` 行にそのまま載るので、識別子でなければコードになる）
3. 端末には次の形だけを打ち込む:
...
7. 専用ディレクトリに残った FIFO は、`initialize` のときとプラグインの終了時に消す。`run.lock` により `totsuka run` は 1 つしか動かず、orca プラグインも 1 つなので、他のインスタンスの FIFO を消すことはない
...
- 検証は 3 段で行う。`handoff` の単体テスト（本物の FIFO と `/bin/sh` で往復）、fake orca CLI の結合テスト（fake が読み手を演じ、読まれた中身と打ち込まれたコマンドを検査する）、実機（[live-e2e-orca](/components/live-e2e-orca.md)）
