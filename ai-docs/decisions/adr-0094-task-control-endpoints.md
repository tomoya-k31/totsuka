...
2. **認証は `/focus` と同じ。** Bearer（`[hooks].auth_token_ref`）と 0600。トークン未設定なら Bearer なしで受け付ける振る舞いも同じ（トークンの出どころは後に [ADR-0099](/decisions/adr-0099-generated-hook-token.md) で `run` が生成するファイルに変わった）
