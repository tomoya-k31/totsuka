...
「plugin restarted since the mention?」を「plugin restart, FIFO eviction, or a rolled-back delivery」に置き換える。当初は「修正後は再起動が残る原因の筆頭になるのだから据え置く」と決めたが、PR レビュー（Copilot）の指摘を容れて撤回した。**残る原因は 3 つあり、そのうち 1 つだけを名指す推測は障害対応を誤った方向へ引っ張る** —— 今回の一件がまさにそれで、当たっていない括弧書きのせいで原因究明が遠回りになった。同じ罠を次の人に残すのは、この ADR が直そうとしている失敗そのものである。
