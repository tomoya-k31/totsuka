...
Socket API の版ではない。7 版を実測すると、**両方向に外れている**:
...
- **上がっても壊れていない**: 17 → 22 の 4 回の bump で、22 メソッドの request
  ...
  変わらない）
- **据え置きでも変わっていない**: 0.9.0 → 0.9.1 は protocol も 22 のままで、
  スライスも完全に同一だった。**protocol が動かないことは「変化が無い」の証拠に
  ならない**（16 → 16 で `custom_status` が消えている）ので、これは偶然一致した
  1 件であって、据え置きを安全の信号にしてよい根拠ではない
- **上がらずに壊れた**: `custom_status` の削除は protocol 16 → 16 で起きた

スキーマ自身の版 `schema_version` も 7 版を通して `1` のままで、これも信号に
...
**下限版の型で読めること自体を D-3 の CI 差分が検査する**。

**その検査が何を保証し、何をしないかは正確に言う必要がある。** 落とすのは
メソッド・result タグ・プロパティの削除、プロパティが写る Rust の型の入れ替え、
result の `required` から外れること、request への `required` 追加、enum バリアントの
削除。**落とさないのは** `pattern` / `maxProperties` / `minimum` のような**制約の
厳格化**（形状ではないので生成に現れない）と、schema に出ない振る舞いの変化
（#521）。「新しい版は追加しかしない」と書くのは、この検査より強い主張になる。
