---
type: Decision
title: ADR-0092 git の呼び出しに上限時間を設け、超えたら git を止めて失敗させる
...
2. **上限を超えたら git を kill し、stdout / stderr の読み取りスレッドを待たずに返す。** git が起動した `ssh` は git が死んでも生き残り、git の stderr を握り続ける。読み取りスレッドを join すると、その ssh が生きている間ずっと戻らない（テストで実測。join すると 30 秒待った）。読み取りスレッドと ssh は置き去りにし、ssh が ServerAlive か OS の TCP keepalive で接続を諦めたときに一緒に消える
...
4. **git は totsuka と同じプロセスグループに置く。** stdio は `Command::output()` と同じ（stdin は `/dev/null`、stdout / stderr はパイプ）。Ctrl-C の SIGINT は端末の前面グループ全体に届くので、固まった git も一緒に止まり、Engine のループが戻って停止を処理できる。git が固まっている間、Engine はシグナルを見に行けないので、これが Ctrl-C で止める唯一の経路になる（#764 の障害で実際に使われた）
...
- **git を自分のプロセスグループに入れ、上限でグループごと SIGKILL する（最初の実装）** — ssh の孤児は残らないが、Ctrl-C の SIGINT が git に届かなくなり、固まった git を抱えた `totsuka run` の停止が上限まで待たされる（`0` なら永久に止まらない）。さらに前面グループの外の ssh は端末から読めず、パスフレーズのプロンプトで SIGTTIN により停止する。`/code-review` の報告から判明し、置き換えた
...
- 打ち切った呼び出しごとに、死んだ接続を待つ ssh と読み取りスレッドが 1 組残る。1 タスクの dispatch は最大 `DISPATCH_RETRY_LIMIT` 回なので、残るのは最大でその数。ssh が接続を諦めたときに消える（推奨の ServerAlive 設定なら約 2 分、無ければ OS の TCP keepalive で約 2 時間）
- Ctrl-C・パスフレーズのプロンプトなど、端末との関係はこの変更の前と変わらない
- 検証は `adapters::git` の単体テストで行う。`core.sshCommand=sleep 30 #` で git の子として固まる ssh を再現し、上限 1 秒で 10 秒以内に `TimedOut` が返ることを確かめる。kill の後に読み取りスレッドを join するように変えると、このテストは 30 秒かかって落ちる
