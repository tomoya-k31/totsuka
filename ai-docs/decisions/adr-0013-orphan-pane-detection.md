...
ただし「orca は `session/list` を呼ばれない」は [ADR-0082](/decisions/adr-0082-orca-herdr-parity.md) が置き換えた。orca も worktree の orca comment `totsuka {task_id}` で所有端末を列挙する（タブタイトルは作業中のエージェントが書き換えるので使わない）。
