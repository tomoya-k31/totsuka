...
generated: { by: claude-code/opus-5, at: 2026-09-14T00:09:46+09:00 }
status: stable
owner: tomoya-k31
---
...
書き込みは**読んでから書き、mutex を読み書きに跨いで持つ**。ドレインループは 2 本走るが単一タスク上で、この関数に `.await` が無いのでプロセス内では今日のところ競合しない —— ロックが買っているのは相互排他ではなく**単調性**である（既にある値より古いタイムスタンプを落とし、ファイルが「最後に書き終えた側」ではなく「**最後の受信**」を答えるようにする）。

**プロセスを跨ぐと保証は無い。** `doctor` と `config validate` はそれぞれ自前のプラグインプロセスを起動し、`initialize` がドレインを始めるので、`run` の稼働中に実行すると数秒間 2 つの書き手が居る。そこは素の競合だが、最悪でも数秒古い受信履歴が残るだけで許容できる。**裂けたファイルは許容できない** —— 読み手は解析失敗を `Receipt::default()`、すなわち「一度も受信していない」に倒すので、**動いているゲートウェイに対して偽の警告が出る**。これはこの仕組みが消そうとしている失敗そのものなので、`atomic_write` の一時ファイル名に pid を入れて排除する。

読み出しは `checked_add` —— ただのファイルなので壊れた値で `SystemTime` の加算が panic しうる。
