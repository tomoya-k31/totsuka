...
機密を扱わない判断の副作用として、**`setup` は回答ファイルにトークンを書かない**。dotfiles に置いても安全である、という性質はこの判断から出ている。これは**書き手の振る舞いの性質であって、形式が保持できないという性質ではない**（#466）: `path` / `summary` / `base_url` 等は自由文字列で、手で編集したファイルには何でも入りうる。`deny_unknown_fields` が禁じるのは未知のキーであって、既知のキーへ機密を貼ることではない。
