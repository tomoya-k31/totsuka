...
**ただしコストの見積もりには 1 点補足が要る。** 監視は `event_source = "gateway"` でも Gateway を通らず `conversations.history` を直接ポーリングする（Gateway は「自分に関係しうるもの」しか publish しないので、監視チャンネルへの投稿はそもそも流れてこない）。したがってこの案の変更は**プラグイン内で閉じ、wire schema のバンプも Gateway の再デプロイも伴わない** —— 上のメンション案より構造的には軽い。退けた理由は費用ではなく、`repo` 固定という監視トリガの設計そのものと、起動時バックフィルが最大 24 時間ぶんの申請を一括でタスク化することにある。**自動起動に舵を切るなら、この案がメンション経路より先に検討されるべきである。**
