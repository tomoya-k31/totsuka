...
ただし正確には、**プロンプトが出うるのは `--online` 固有ではない**（レビューでの指摘）。プラグインが 1 つでも enabled なら `check_plugins` がプラグイン起動のために `plugin_spec` → `llm_info` / `plugin_init_config` を呼び、`[llm].api_key_ref` と `plugins/{name}.toml` のシークレットを `op://` 含めて実解決する。つまり **ADR-0006 の「doctor は非対話」は `check_llm_key` 単体では守られているが doctor 全体では既に破れている**（#267 以前からの既存挙動）。本 ADR ではこれを是正しない — `--online` が新たに壊すものは無く、範囲が別だからである。**ネットワークに出るのは `llm-online` だけ**という点は本 ADR で正しく成立する。既存のギャップを塞ぐかどうかは別途判断する。
