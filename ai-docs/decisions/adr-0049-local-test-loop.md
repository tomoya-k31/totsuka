...
§5 が挙げた不採用理由は 4 つで、そのうち 3 つ（CI の install ステップ・設定ファイル・
doctest の別ステップ化）は**ローカル限定にした時点で消える**。残る 1 つ「ローカル
`cargo test` との乖離」だけが本 ADR にそのまま残るので、明示的に引き受ける:

- **乖離の向きは一方向である。** nextest はテストごとにプロセスを分けるので、
  libtest（プロセス共有）が通すものを落とすことはあっても、逆は起きにくい。
  実際 §3 の env 命名バグを検出したのは nextest のほうである。
- したがってローカルで緑・CI で赤という事故は理屈の上では起こりうるが、
  **最終ゲートは CI の `cargo test`** で、そこは何も変わっていない。ローカルが
  先に落ちる分には早く気づくだけで、ゲートは弱くならない。
- 逆向き（ローカルで落ちて CI で通る）は、プロセス共有前提の隠れた結合を
  暴いた場合に起きる。それは直すべきものなので、乖離ではなく検出と扱う。
