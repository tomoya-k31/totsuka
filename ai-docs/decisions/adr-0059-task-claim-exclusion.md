...
>
> **機構の注記**: 本文の `assignable_to_me`（プラグイン全体の assignee ゲート）は [ADR-0063](/decisions/adr-0063-trigger-assignee.md) が `trigger.assignee` へ移し、削除した。既定は同じ条件なので、この ADR の前提は変わらない。
