...
前提: 対象リポジトリを `orca repo add --path <repository>` で登録しておき、そのリポジトリ設定で external worktree を表示にしておく（`orca repo show --json` の `externalWorktreeVisibility` が `show`）（[ADR-0081](/decisions/adr-0081-orca-herdr-parity.md)）。
