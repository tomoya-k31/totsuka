...
| プラグイン適合（黒箱） | 公式 7 本それぞれの `tests/conformance.rs` | [plugin-conformance](/components/plugin-conformance.md) がプラグインの**実バイナリ**を起動し、stdio の NDJSON で全プラグイン共通の約束事（initialize 前の拒否・PARSE_ERROR・METHOD_NOT_FOUND・空行と通知に無応答・INVALID_PARAMS・`config/validate` の未知キー・`shutdown` と EOF での終了・task_source の未知トリガーキー）を検査する（#767、[ADR-0104](/decisions/adr-0104-plugin-conformance-kit.md)）。プラグインのテストは `plugin_conformance::check` の 1 本で、違反を全部並べて落ちる。`initialize` が成功した後の経路は実 API に触れるので対象外で、各プラグインの in-process テストが持つ。汎用挙動のロジックそのものは `plugin-sdk` の `tests/sdk.rs` が SDK のサーバーに対して検査しており、この層が見るのは「そのバイナリが SDK に正しく配線されているか」である |
