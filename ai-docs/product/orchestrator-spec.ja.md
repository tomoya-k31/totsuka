...
| ID | 要件 | 優先度 |
|---|---|---|
| F-01 | タスクソースをプラグインとして接続し、タスク一覧・詳細を正規化された共通スキーマ(Task)で取得できる。ソースは `task/submit` でそのタスクが属する workflow を名指す(0.6.0、#554) —— 受け取った workflow 群に対して first-match を既に走らせているためで、Orchestrator はその名前が実在しそのソースのものかだけを検証する | M |
...
| F-45 | dispatch 後で未完了の状態はすべて同時実行上限のカウント対象とする: `dispatched → running → verifying → publishing` に加え、**人間待ちの `waiting_input` と `escalated` も数える**(ADR-0093)。待っている間もスロットを保持し、再開はそのスロットのまま行うので、上限は「人間待ちを含めて同時に開いているタスクの数」を縛る。`pending`(リポジトリ確認待ち)は dispatch 前なのでスロットを持たない | M |
...
| F-56 | **install(バイナリの存在)と enabled(設定の宣言)を分離**する。install 先は `$XDG_DATA_HOME/totsuka/plugins/{name}/`、有効/無効は config.toml の `[plugins.{name}] enabled` フラグで宣言的に管理する。このロスターは、トップレベルの `[<name>]` 設定テーブルを正当と認める根拠でもある(F-64) | M |
...
| F-59 | プラグイン固有設定の検証は、プラグイン側に必須の `config/validate` RPC メソッドを設けて委譲する。`config validate` 実行時、有効な全プラグインを一時起動して検証させる(socket 疎通確認などスキーマで表現できない検証を可能にするため)。0.6.0 以降はそのプラグインに渡した workflows / projects / repositories も同じ呼び出しで運ぶので、記憶ではなく「今聞かれているもの」を検証する | M |
...
```toml
# config.toml — ロスター(Orchestrator が解釈する)
[plugins.herdr]
enabled = true
kind = "agent_ide"
max_concurrency = 3
...
| F-64 | プラグイン個別設定は **config.toml** のトップレベル `[<name>]` テーブルに置き、Orchestrator は無解釈で保持する。`[plugins.*]` のロスターに無い名前のテーブルは検証エラーで、core キーのタイポもプラグイン名のタイポも同じ検査で落ちる。プラグインが *core の構造体*(`[[workflows]]` / `[[projects]]`)に生やすキーはフラットに書き、どのプラグインが消費するかを聞いて解決する —— [ADR-0058](/decisions/adr-0058-config-ownership-boundary.md) 参照。#554 以前は `plugins/{name}.toml` への分離を要求しており、所有をファイル位置で表現していたため core の構造体の中へ届かなかった | M |
...
| F-66 | 設定の優先順位: CLI フラグ > 環境変数 > プラグイン自身の `[<name>]` テーブル > `config.toml` のデフォルト値 | M |
| F-67 | リポジトリの起票先トラッカーはトップレベルの `[[projects]]` で宣言し(`name` / `source` と、そのプラグイン自身のキーを無解釈で保持)、リポジトリは `[[repositories]].project` で**1つ**を名指す。任意 —— トラッカーの無いリポジトリは正常な状態である。`source` を書かせるのは、`repositories.project` → `projects.name` → `plugins.<source>` の連鎖をプラグインを起動せずに辿れるようにするため。各ソースが持っていた逆引きの `repos = [...]` を置き換える(#554、[ADR-0058](/decisions/adr-0058-config-ownership-boundary.md)) | M |
...
| F-108 | **質問ダイアログの park**(#487、ADR-0050): design / implement(attended)profile は人間への質問・完了確認をツール native の質問 UI で行う — claude は `AskUserQuestion`(この 2 profile の settings にだけ描画される `PreToolUse` フック)、opencode は `question`(opencode v2 でプラグインが受ける `form.created` イベント。ADR-0105)。codex は Default mode に質問ツールが無いため `NEEDS_INPUT` + 番号付き選択肢リストのまま。ダイアログ待機中はターンが終わらず `Stop` が届かないので、フックが `QuestionPending`(質問ごとの `prompt_id` 必須 — 空だと 2 問目が dedup で消える)を POST し、Engine は `Stop{NEEDS_INPUT}` と同様に `waiting_input` へ park する: 質問文つき通知(スロットは保持、F-45)。park 中の**新しい**質問は再通知される。マーカーは完了ワイヤシグナルのまま(ADR-0020)で、質問ツール不能時のフォールバック(`NEEDS_INPUT` + 番号付きリスト)はプロンプト内に残る | M |
...
| F-110 | **ランタイム health(`health.json`)**: `run` は毎サイクル `$XDG_STATE_HOME/totsuka/health.json` を temp+rename で置き換え、graceful shutdown で削除する。内容は**毎サイクル問い直せる縮退だけ** — hook receiver の bind 失敗 / プラグインの停止(`abandoned` かどうかで対処が変わる) / spool バックログ(`*.jsonl` のみ。`.corrupt` は自動回収されないので数えない) / LLM 鍵の 401·403(成功で解除するラッチ) / LLM ゲートウェイの到達不能(F-111。答えが返れば解除するラッチ)。一過性の失敗(notify 配送失敗・掃除失敗)は再評価できず永久に消えないため入れない。散文は保存せず読み出し側が組み立て、未知の `kind` も行として残す。`totsuka status` は `degraded:` ブロック(`--json` は `health`)、`totsuka menu` は `⚠` として読む。**`run.lock` が優先**で、停止中の run が残した文書は無視する(pid も突き合わせる) — 落ちた run の health は存在しないプロセスの話だからである。さらに読み手は**鮮度**も見る: pid は生きているのに 120 秒 republish が無い文書は stale として扱い、**捨てずに 1 つの縮退として出す**(捨てると、黙った run について「健全」と報告することになる)。stale は run が publish するものではなく読み手の判断で、`status --json` は `health.recorded_at` と `health.stale` の両方を出す。state.db でなくファイルなのは、スキーマを上げると古い totsuka が DB を開けなくなる(ADR-0017)ため | S |
| F-111 | **LLM ゲートウェイの生存確認**: `run` は最後の接触(実呼び出しまたはプローブ)から 10 分、`llm_unreachable` ラッチ中は 60 秒沈黙したら、`doctor --online` と同じ最小リクエスト(schema なし・リトライなし・`max_tokens: 1`)を **spawn して**投げ、結果を `health.json` の `llm_unreachable`(接続不可・タイムアウト・5xx。理由の短文を持ち、応答本文は載せない。**ゲートウェイから何か答えが返れば 401 でも解除**)と `llm_key_rejected` に折り込む。接触が一度も無ければ即時 = **起動時に必ず 1 回**。ループはプローブを待たない(タイムアウト分止まらない)。**復帰検知**: サイクル間で壁時計が単調時計より 30 秒以上先へ進んだら「スリープから復帰」とみなし、HTTP 接続プールを捨てて即プローブする(プールの idle timeout は単調時計なのでスリープ中に効かない)。429 と 401/403 以外の 4xx は「答えている」ので到達不能にしない。起動は止めない(LLM 無しでも `pending` で動く設計に合わせる)。設定キーは持たない。task_source プラグインが自前で呼ぶ LLM は対象外(ADR-0070) | S |
